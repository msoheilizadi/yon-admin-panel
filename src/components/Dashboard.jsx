...
export default function Dashboard() {
  ...
  const [formData, setFormData] = useState({
    title: '', subtitle: '', description: '', category: '', album: '', duration: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ title: '', subtitle: '', description: '', category: '', album: '', duration: '' });
      setImageFile(null);
      setAudioFile(null);
      fetchSongs();
    } catch (err) {
  ...
  if (loading) return <div className="admin-container" style={{ textAlign: 'center' }}>در حال بارگذاری...</div>;
    ...
    <div className="admin-container">
      ...
      <div className="add-song-section">
        ...
        <form onSubmit={handleSubmit} className="song-form" encType="multipart/form-data">
          <input name="title" placeholder="عنوان" value={formData.title} onChange={handleChange} required />
          <input name="subtitle" placeholder="زیر عنوان" value={formData.subtitle} onChange={handleChange} />
          <textarea name="description" placeholder="توضیحات" value={formData.description} onChange={handleChange} rows="3" className="full-width" />
          <input name="category" placeholder="دسته‌بندی" value={formData.category} onChange={handleChange} />
          <input name="album" placeholder="آلبوم" value={formData.album} onChange={handleChange} />
          <input name="duration" type="number" placeholder="مدت (ثانیه)" value={formData.duration} onChange={handleChange} />
      ...
      <div className="songs-list">
        <h2>📋 لیست آهنگ‌ها</h2>
        {songs.length === 0 && <p style={{ textAlign: 'center', color: 'var(--text)' }}>هیچ آهنگی یافت نشد</p>}
